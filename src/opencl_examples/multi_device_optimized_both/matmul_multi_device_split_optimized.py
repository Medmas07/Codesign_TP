...
Work split (rows):
  - NVIDIA GPU: Kernel 6 optimized kernel (tiling + register blocking)
  - Intel iGPU: Kernel 6 optimized kernel (tiling + register blocking)
...
def run_device_kernel6(
    ...
    """
    Execute Kernel 6 style SGEMM on one device for one split:
      A_sub [M, N] x B_full [N, N] => C_sub [M, N]
    B is transposed once on host and sent as BT to improve access patterns.
    Uses kernel-only timing (OpenCL event profiling).
    H2D is done once before timing, D2H once after timing.
    """
    if runs < 1:
    ...
    program = build_program_with_log(ctx, KERNEL6, options=build_options)
    kernel = program.mmul_kernel6
    kernel.set_scalar_arg_dtypes([np.int32, np.int32, None, None, None])

    mf = cl.mem_flags
    d_a = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=a_sub)
    d_bt = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=bt_full)
    ...
    def one_pass_kernel_sec() -> float:
        evt = kernel(queue, global_size, local_size, np.int32(n), np.int32(m), d_a, d_bt, d_c)
        evt.wait()
        return (evt.profile.end - evt.profile.start) * 1.0e-9
    ...
    run_times = []
    for _ in range(runs):
        run_times.append(one_pass_kernel_sec())

    cl.enqueue_copy(queue, c_last, d_c).wait()
...
def split_and_run(
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        fut_nv = pool.submit(run_device_kernel6, a0, b, runs, warmup, nvidia_dev) if a0.shape[0] > 0 else None
        fut_in = pool.submit(run_device_kernel6, a1, b, runs, warmup, intel_dev) if a1.shape[0] > 0 else None
    ...
    c = np.vstack(parts) if parts else np.empty((0, n), dtype=np.float32)
    ...
    print("\nGlobal timing (kernel-only):")
    print(f"  Total time (parallel estimate, {runs} runs): {total_time:.6f} s")
    print(f"  Mean time per run: {mean_time:.6f} s")
    print(f"  Global GFLOPS: {gflops_total:.2f}")
    print(f"  Host orchestration wall time (with setup/build): {orchestration_wall:.6f} s")

    # Baseline: NVIDIA only on full matrix, same runs/warmup, using Kernel 6.
    print("\nRunning NVIDIA-only baseline (full matrix, Kernel 6)...")
    base_nv = run_device_kernel6(a, b, runs=runs, warmup=warmup, device=nvidia_dev)
    baseline_time = base_nv.total_time
    baseline_mean_time = baseline_time / runs
    baseline_gflops = (total_flops / baseline_time) / 1e9
    speedup = baseline_time / total_time

    print(f"  Baseline NVIDIA-only time: {baseline_time:.6f} s")
    print(f"  Baseline NVIDIA-only mean time per run: {baseline_mean_time:.6f} s")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PyOpenCL multi-device SGEMM split (Kernel 6 on NVIDIA + Intel iGPU).")
